...
const insertMember = async () => {
	...
	for (let i = 0; i < numOfNewDoc; i++) {
		const departList = Object.keys(DEPARTMENT).map(
			(key) => DEPARTMENT[key as keyof typeof DEPARTMENT]
		);
		...
		const member: Member = {
			...
			admin: Array.from({ length: Math.random() * maxLengthOfAdmin }, () =>
				uuidv4()
			),
		};
...
const insertArtwork = async () => {
	...
	const numOfNewDoc = 10;
	for (let i = 0; i < numOfNewDoc; i++) {
		const typeList = Object.keys(ARTWORK_TYPE).map(
			(key) => ARTWORK_TYPE[key as keyof typeof ARTWORK_TYPE]
		);
...
const insertEvent = async () => {
	...
	for (let i = 0; i < numOfNewDoc; i++) {
		...
		const typeList = Object.keys(EVENT_TYPE).map(
			(key) => EVENT_TYPE[key as keyof typeof EVENT_TYPE]
		);
