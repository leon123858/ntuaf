...
const TextBox = ({ text }) => {
	...
	return {
		[BlOCK_TYPE.TEXT_A]: (
			<div>
				...
				<div
					...
					style={
						text.title
							? { paddingLeft: 20, marginTop: 44, paddingRight: 20 }
							: { paddingLeft: 20, marginTop: 32, paddingRight: 20 }
					}
				>
					<h1 className={style.one}>{text.url}</h1>
					<div
						className={style.content}
						style={text.title ? { marginTop: 24 } : {}}
					>
						{text.text.split('\n').map((str, idx) => {
		...
		[BlOCK_TYPE.TEXT_B]: (
			...
			>
				{text.title ? <h1 className={style.two}>{text.title}</h1> : ''}
		...
		[BlOCK_TYPE.TEXT_D]: (
			<div
				style={
					inBreakPoint
						? { padding: '50px 0' }
						: { display: 'flex', alignItems: 'center', padding: '50px 0' }
				}
				className={style.container}
			>
				<div style={{ textAlign: 'center' }}>
					<img
						src={text.url}
						alt=''
						style={{ width: 140, marginRight: '30px' }}
					/>
				</div>
